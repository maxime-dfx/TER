...
// MISE À JOUR : Initialisation de deltaT
Solver::Solver(const Mesh& m, const MaterialManager& mat, bool planeStress, double dT) 
    : mesh(m), matMgr(mat), isPlaneStress(planeStress), deltaT(dT) {
...
void Solver::assemble() {
    ...
    #pragma omp parallel
    {
        ...
        // NOUVEAU : Vecteur force local pour éviter les "Race Conditions" en OpenMP
        VectorXd local_F = VectorXd::Zero(numDof);

        #pragma omp for
        for (int t = 0; t < (int)mesh.getTriangles().size(); ++t) {
            Matrix<double, 6, 6> Ke = computeElementStiffness(t);
            Matrix<double, 6, 1> Fe_th = computeElementThermalForce(t); // NOUVEAU
            ...
            for (int i=0; i<6; ++i) {
                // NOUVEAU : Assemblage de l'effort thermique
                local_F(gdof[i]) += Fe_th(i); 
        ...
        #pragma omp critical
        {
            ...
            F += local_F; // NOUVEAU : Addition sécurisée au vecteur force global
        }
    }
...
void Solver::applyBoundaryConditions() {
    ...
    // --- 1. Conditions de Dirichlet Classiques ---
